...
class RegistraForm(FlaskForm):
    ...
    identification = StringField('Identificação (RA/CPF)', validators=[
        DataRequired(message='Este campo é obrigatório.'), 
    ...
    def validate_identification(self, identification):
        user = Usuario.query.filter_by(
            identification=identification.data).first()
        if user:
...
class AtualizaPerfilForm(FlaskForm):
    ...
    confirm_password = PasswordField('Confirmar Senha', validators=[
        DataRequired(message='Este campo é obrigatório.'), 
        EqualTo('password', message='Os dois campos não são identicos.')])
    picture = FileField('Atualizar Foto de Perfil', validators=[
